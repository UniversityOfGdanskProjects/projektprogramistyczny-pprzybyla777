...
const FiveDigitsMaxBeforeFloatingPointAndUpToTwoDigitsAfter = (value) => /^(\d{1,5}|\d{0,5}\.\d{1,2})$/.test(value)
...
export const PizzaFormSchema = yup.object().shape({
  name: yup.string().min(1).max(20, "Name must be 20 characters or less").required("Required"),
  toppings: yup.string().min(3).required("Required"),
  small: yup.number().positive().required("Required").test("small", "Please enter a valid price", FiveDigitsMaxBeforeFloatingPointAndUpToTwoDigitsAfter),
  large: yup.number().positive().required("Required").test("large", "Please enter a valid price", FiveDigitsMaxBeforeFloatingPointAndUpToTwoDigitsAfter),
